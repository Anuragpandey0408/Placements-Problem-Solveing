...
public class MPCSExam {
    public static void main(String[] args) {
        ...
        int[] avg = new int[3];
        System.out.println("Enter oxygen levels for 3 trainees (3 rounds each):");
        ...
        int maxAvg = Math.max(avg[0], Math.max(avg[1], avg[2]));
        if (maxAvg < 70) {
            System.out.println("All trainees are unfit.");
        } else {
            System.out.println("Most fit trainee(s):");
            for (int i = 0; i < 3; i++) {
                if (avg[i] == maxAvg) System.out.println("Trainee Number : " + (i + 1));
            }
            System.out.println("Highest Average Oxygen Level: " + maxAvg);
        }
        sc.close();
    }
}